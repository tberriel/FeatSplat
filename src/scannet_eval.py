...
for n_classes, lambda_sem in [[0,0.0],[64,0.001]]:
    ...
    # Compute averages
    avg_ssim = sum([result[0] for result in results]) / len(results)/iterations
    avg_lpips = sum([result[1] for result in results]) / len(results)/iterations
    avg_psnr = sum([result[2] for result in results]) / len(results)/iterations
