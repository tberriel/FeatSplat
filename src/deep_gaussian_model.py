...
from modules.gaussian_splatting.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from modules.gaussian_splatting.utils.system_utils import mkdir_p
#from src.modules.gaussian_splatting.utils.sh_utils import RGB2SH
from simple_knn._C import distCUDA2
from modules.gaussian_splatting.utils.graphics_utils import BasicPointCloud
from modules.gaussian_splatting.utils.general_utils import strip_symmetric, build_scaling_rotation

from modules.gaussian_splatting.scene.gaussian_model import GaussianModel
...
class DeepGaussianModel(GaussianModel):
    ...
    def get_features(self):
        ...
        sh_features = self.nn_forward(self.latent_features)
        return sh_features
    ...
    def nn_forward(self, latent_features):
        sh_features = self.mlp(latent_features).reshape((latent_features.shape[0],(self.active_sh_degree + 1) ** 2,3)).contiguous()*15
        return sh_features
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [x for x in self.mlp.parameters()], 'lr': training_args.feature_lr, "name": "mlp"}
        ]
        #for i,param in enumerate(self.mlp.parameters()):
        #    l.append({'params': [param], 'lr': training_args.feature_lr, "name": "mlp_"+str(i)})
        """
        for i,param in enumerate(self.mlp.parameter()):
            l.append({'params':param, 'lr': training_args.feature_lr, "name": "mlp_"+str(i)}) 
        """
