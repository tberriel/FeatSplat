    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        while network_gui.conn != None:
            try:
                ...
                if custom_cam != None:
                    net_image = render(custom_cam, gaussians, pipe, background, scaling_modifer, override_color=gaussians.get_features)["render"]
        ...
        with torch.no_grad():
            # Progress bar
            ema_loss_for_log = 0#0.4 * loss.item() + 0.6 * ema_loss_for_log
            if iteration % 10 == 0:
