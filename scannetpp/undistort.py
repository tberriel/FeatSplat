...
def undistort_frames(
    ...
    for frame in tqdm(frames, desc="frame"):
        ...
        # Undistort depth 
        if input_depth_dir is not None:
            file_name = frame["file_path"].replace('.JPG', '.png') # Depth map is png format
            ...
            undistorted_depth = cv2.remap(
                ...
                borderValue=0 # Use 0 as value for unvalid pìxels
            )
        ...
        # Undistort semantics 
        if input_sem_dir is not None:
            file_name = frame["file_path"].replace('.JPG', '.png') # Depth map is png format
            sem_path = Path(input_sem_dir) / file_name
            sem = cv2.imread(str(sem_path), cv2.IMREAD_UNCHANGED)
            undistorted_sem = cv2.remap(
                sem,
                map1,
                map2,
                interpolation=cv2.INTER_NEAREST, # Use nearest neighbor interpolation
                borderMode=cv2.BORDER_REFLECT_101,
                #borderMode=cv2.BORDER_CONSTANT,
                #borderValue=65535 # value assigned to pixels without label
            )
            out_sem_path = Path(out_sem_dir) / file_name
            out_sem_path.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(out_sem_path), undistorted_sem)
        # Mask
...
def main(args):
    ...
    for scene_id in tqdm(scene_ids, desc="scene"):
        ...
        if input_depth_dir is not None:
            ...
            out_depth_dir = scene.dslr_dir / cfg.get("out_depth_dir")

        input_sem_dir = cfg.get("input_sem_dir", None)
        out_sem_dir = None
        if input_sem_dir is not None:
            input_sem_dir = scene.dslr_dir / input_sem_dir
            out_sem_dir = scene.dslr_dir / cfg.get("out_sem_dir")
