...
def main(args):
    ...
    for scene_id in tqdm(scene_ids, desc="scene"):
        ...
        semantic_mesh = plydata.elements[0]['label']
        for device in render_devices:
            ...
            semantic_dir = Path(cfg.output_dir) / scene_id / device / "render_semantic"
            ...
            for image_id, image in tqdm(images.items(), f"Rendering {device} images"):
                ...
                # Select labels using majority vote for classes
                vert_labels = np.where(vert_indices>0, semantic_mesh[vert_indices], -100)
                #semantic = scipy.stats.mode(vert_labels, axis=-1).mode 
                semantic = vert_labels[...,0]# Mode is extremeley slow, to be faster we just select the first of the vertex labels, although we will lose accuracy
