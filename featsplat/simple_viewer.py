...
def load_gaussian(path, iteration=30000):
    with open(os.path.join(path,"cfg_args"), 'r') as f:
        cfg = f.read()
    args = eval(cfg)
    
    pc = FeatGaussianModel(args)
    pc.load_ply(os.path.join(path, "point_cloud", "iteration_" + str(iteration),"point_cloud.ply"))
    ckpt = {
        "means":pc.get_xyz,
...
def mlp_forward(projected_features, camera_pos, mlp):
        ...
        rendered_image = mlp(x)[None][...,:3].reshape((h,w,3))
